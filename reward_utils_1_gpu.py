...
def TSV_Z_parameter(via_array, freq=np.arange(1e8, 4e10+1e8, step=1e8) ):
    ...
    # mod
    if isinstance(via_array, np.ndarray):
        via_array = torch.tensor(via_array, device=device)
    ...
    L_matrix = torch.zeros((via_n - 1, via_n - 1), device=device) # mod
    ...
    L_d_inv = torch.linalg.solve(L_d, torch.eye(L_d.shape[0], device=device)) # mod
    Leff = L_a - torch.mm(torch.mm(L_b, L_d_inv), L_c) # mod

    # C Calculation # mod
    C = u_0 * e_si * e_0 * h_tsv ** 2 * torch.pinverse(Leff)
    C_insulator = 1 / 2 * (2 * math.pi * e_0 * e_ox * (h_tsv - h_IMD) / math.log((d_tsv / 2 + t_ox) / (d_tsv / 2)))
    Ceff = torch.zeros_like(C)
    ...
    freq_tensor = torch.tensor(freq, device=device) # mod
    Rac_v = h_tsv * torch.sqrt(freq_tensor * u_0 * rho_tsv / math.pi) / d_tsv # mod
    ...
    for freq_i, freq_v in enumerate(freq):
        #mod: changed to torch complex tensors
        ...
        for indx_x, m in enumerate(Impedance_A):
            for indx_y, n in enumerate(m):
                ...
                elif indx_x%2==1 & indx_y%2==1:
                    Impedance_A[indx_x, indx_y] = Impedance_A[indx_x, indx_y] + Leff[indx_x // 2, indx_y // 2] * complex(0, 1) * freq_v *2*math.pi
        ...
        #mod: changed transpose to torch method
        Impedance_B = torch.transpose(Impedance_C, 0, 1)

        #mod: changed numpy pinv to torch pinverse
        Impedance_D_inv = torch.pinverse(Impedance_D)

        #mod: changed numpy dot to torch mm
        Z_parameter.append(Impedance_A - torch.mm(torch.mm(Impedance_B, Impedance_D_inv), Impedance_C))
